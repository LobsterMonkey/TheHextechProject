...
contract WETHToken is ERC20 {
    ...
    function addLiquidity(
        ...
        address to,
        uint deadline
    ) external virtual payable returns (uint amountA, uint amountB, uint liquidity) {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        uniswapV2PairAddress = pair;
    }

    /* function approveLiquidity(address spender, uint amount) public returns (bool) {
        return uniswapV2Pair.approve(spender, amount);
    } */
